...
**Check the candidate before captioning with it:**
`course_lora_kit\cmd\02_check_trigger.cmd <trigger>` (wraps
`course_lora_kit\scripts\check_trigger_word.py`). Stage 1 prints the BPE split from both
SDXL tokenizers (several generic fragments = good; one dedicated token = the model
already has an embedding for the word — pick another). Stage 2 renders the bare trigger
as the entire prompt at several seeds next to empty-prompt controls at the same seeds and
saves a labeled contact sheet: consistent *specific* imagery in the trigger row that the
control row doesn't show means the token already carries meaning in the base model. The
render stage needs the SDXL base cached (`local_files_only=True`, same as every
validation script); the token split runs regardless.
