...
Scripts live in `course_lora_kit/scripts/` (verbatim copies of the course repo's
`course_v3/appendices/assets/`; that folder's README is the canonical deep-dive). Each
has a `.cmd` wrapper in `course_lora_kit/cmd/` that runs it in OneTrainer's venv:
