...
| Script | Args |
| --- | --- |
| `00_verify_setup.cmd` | — (venv + CUDA + key-file check; first run asks for `LORA_ROOT` once, writes the gitignored `course_lora_kit\local_paths.cmd`, creates `%LORA_ROOT%\dataset\{trigger,notrigger}` and both `training_concepts\*_concepts.json`) |
...
| `03_train_character_sdxl.cmd` | [extra `--config-value` flags]; needs `training_concepts\character_concepts.json` (written by `00_verify_setup.cmd`) |
